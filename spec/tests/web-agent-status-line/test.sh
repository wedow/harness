...
# Running driver + one queued waiter on the session lock.
( exec 9>"${dir}/.lock"; flock 9; sleep 2 ) &
holder=$!
( exec 9>>"${dir}/.lock"; flock 9; sleep 0.3 ) &
waiter=$!
sleep 0.5

line="$(_agent_status_line "${sid}")"
[[ "${line}" == *"1 subagent"* ]] || { echo "FAIL: expected subagent in: ${line}"; kill ${holder} ${waiter} ${sub_pid} 2>/dev/null || true; exit 1; }
[[ "${line}" == *"1 queued"* ]] || { echo "FAIL: expected queued in: ${line}"; kill ${holder} ${waiter} ${sub_pid} 2>/dev/null || true; exit 1; }
[[ "${line}" != *"2 subagent"* ]] || { echo "FAIL: counted finished subagent: ${line}"; kill ${holder} ${waiter} ${sub_pid} 2>/dev/null || true; exit 1; }

kill ${holder} ${waiter} ${sub_pid} 2>/dev/null || true
wait ${holder} ${waiter} 2>/dev/null || true
