...
assert_json '.messages[0].type' "$out" "function_call_output"
assert_json '.messages[0].call_id' "$out" "call_new"
assert_json '.messages[0].output' "$out" $'tool output before nested dump\n---\nrole: tool_result\ncall_id: call_old\ntool: bash\nerror: false\n---\ntool output after nested dump'
